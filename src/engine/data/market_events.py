...
from engine.io.paths import market_events_dir


# -----------------------------------------------------------------------------
# data/market_events writer
#
# Contract:
...
# - Expands df to full schema contract via enforce_table("market_events")
# -----------------------------------------------------------------------------
...
def validate_market_events_frame(df: pl.DataFrame) -> None:
    """
    ...
    if df is None or df.is_empty():
        return
    missing = [c for c in ("instrument", "anchor_tf", "ts", "event_type") if c not in df.columns]
    if missing:
        raise ValueError(
            "data/market_events frame missing key columns: "
            f"{missing} columns_present={sorted(df.columns)} rows={df.height}"
        )


def write_market_events_for_instrument_tf_day(
    *,
    ctx: RunContext,
    ...
    validate_market_events_frame(out)

    out = enforce_table(out, "market_events", allow_extra=True, reorder=True)

    out_dir = market_events_dir(
        ctx=ctx,
