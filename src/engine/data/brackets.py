...
from engine.io.paths import brackets_dir
...
def _ensure_identity_columns(ctx: RunContext, trading_day: date, df: pl.DataFrame) -> pl.DataFrame:
    ...
    if _is_eval_mode(out):
        if "principle_id" not in out.columns:
            raise ValueError("brackets: eval-mode requires principle_id when paradigm_id is present")
...
def _validate_brackets_frame(df: pl.DataFrame) -> None:
    if df is None or df.is_empty():
        return
    required = ["instrument", "dt"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"brackets frame missing required columns: {missing}")


def write_brackets_for_instrument_day(
    *,
    ctx: RunContext,
    trading_day: date,
    instrument: str,
    ...
    df = _ensure_identity_columns(ctx, trading_day, brackets_df)
    ...
    for g in parts:
        ...
        g2 = enforce_table(g, "brackets", allow_extra=True, reorder=False)
        write_parquet(g2, out_dir, file_name=file_name)

    log.info("brackets.write_brackets_for_instrument_day: wrote %d rows across %d partition(s) for dt=%s", df.height, len(parts), trading_day)


def read_brackets_for_instrument_day(
    *,
    ctx: RunContext,
    trading_day: date,
    instrument: str,
