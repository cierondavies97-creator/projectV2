...
def build_feature_frame(
    ...
    **_,
    ...
    for anchor_tf in anchor_tfs:
        ...
        df = df.with_columns(
            ((pl.col("fvg_upper") + pl.col("fvg_lower")) / pl.lit(2.0))
            .cast(pl.Float64, strict=False)
            .alias("fvg_mid")
        )
        ...
        df = df.with_columns(
            pl.when(pl.col("fvg_direction") != pl.lit("none"))
            .then(pl.col("_bar_index"))
            .otherwise(pl.lit(None))
            .alias("fvg_origin_idx"),
            pl.when(pl.col("fvg_direction") != pl.lit("none"))
            .then(pl.col("ts"))
            .otherwise(pl.lit(None))
            .alias("fvg_origin_ts"),
        ...
        df = df.with_columns(
            pl.when(pl.col("fvg_direction") != pl.lit("none"))
            .then(pl.lit(str(anchor_tf)))
            .otherwise(pl.lit(None))
            .alias("fvg_origin_tf")
        )

        df = df.with_columns(
            pl.col("fvg_origin_idx").forward_fill().over("instrument").alias("fvg_origin_idx"),
            pl.col("fvg_lower").forward_fill().over("instrument").alias("fvg_lower"),
            pl.col("fvg_upper").forward_fill().over("instrument").alias("fvg_upper"),
            pl.col("fvg_mid").forward_fill().over("instrument").alias("fvg_mid"),
            pl.col("fvg_origin_ts").forward_fill().over("instrument").alias("fvg_origin_ts"),
            pl.col("fvg_origin_tf").forward_fill().over("instrument").alias("fvg_origin_tf"),
        )

        df = df.with_columns(
            (pl.col("_bar_index") - pl.col("fvg_origin_idx"))
            .cast(pl.Int32, strict=False)
            .alias("fvg_age_bars")
        )

        df = df.with_columns(
            pl.when(pl.col("fvg_age_bars") > pl.lit(int(cfg.fvg_max_origin_age_bars)))
            .then(pl.lit("none"))
            .otherwise(pl.col("fvg_direction"))
            .alias("fvg_direction")
        )
        ...
        df = df.with_columns(
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit(None))
            .otherwise(pl.col("fvg_lower"))
            .alias("fvg_lower"),
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit(None))
            .otherwise(pl.col("fvg_upper"))
            .alias("fvg_upper"),
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit(None))
            .otherwise(pl.col("fvg_mid"))
            .alias("fvg_mid"),
        )

        origin_range_atr_expr = safe_div(
            (pl.col("high").shift(1).over("instrument") - pl.col("low").shift(1).over("instrument")).abs(),
            pl.col("atr_anchor").shift(1).over("instrument"),
            default=0.0,
        ).alias("fvg_origin_impulse_atr")

        origin_body_ratio_expr = safe_div(
            (pl.col("close").shift(1).over("instrument") - pl.col("open").shift(1).over("instrument")).abs(),
            (pl.col("high").shift(1).over("instrument") - pl.col("low").shift(1).over("instrument")).abs(),
            default=0.0,
        ).alias("fvg_origin_body_ratio")

        df = df.with_columns(origin_range_atr_expr, origin_body_ratio_expr)

        df = df.with_columns(
            pl.when(pl.col("fvg_direction") != pl.lit("none"))
            .then(pl.col("fvg_origin_impulse_atr"))
            .otherwise(pl.lit(None))
            .alias("fvg_origin_impulse_atr"),
            pl.when(pl.col("fvg_direction") != pl.lit("none"))
            .then(pl.col("fvg_origin_body_ratio"))
            .otherwise(pl.lit(None))
            .alias("fvg_origin_body_ratio"),
        )

        df = df.with_columns(
            pl.col("fvg_origin_impulse_atr").forward_fill().over("instrument").alias("fvg_origin_impulse_atr"),
            pl.col("fvg_origin_body_ratio").forward_fill().over("instrument").alias("fvg_origin_body_ratio"),
        )

        df = df.with_columns(
            pl.when(
                (pl.col("fvg_origin_impulse_atr") >= pl.lit(float(cfg.fvg_origin_impulse_range_atr_min)))
                & (pl.col("fvg_origin_body_ratio") >= pl.lit(float(cfg.fvg_origin_body_ratio_min)))
            )
            .then(pl.col("fvg_direction"))
            .otherwise(pl.lit("none"))
            .alias("fvg_direction")
        )
        ...
        df = df.with_columns(
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit(0.0))
            .otherwise(pl.col("fvg_gap_ticks"))
            .cast(pl.Float64, strict=False)
            .alias("fvg_gap_ticks")
        )

        gap_size_expr = (pl.col("fvg_upper") - pl.col("fvg_lower")).abs()
        overlap_expr = (
            pl.min_horizontal(pl.col("high"), pl.col("fvg_upper"))
            - pl.max_horizontal(pl.col("low"), pl.col("fvg_lower"))
        ).clip_min(0.0)

        df = df.with_columns(
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit(0.0))
            .otherwise(safe_div(overlap_expr, gap_size_expr, default=0.0))
            .cast(pl.Float64, strict=False)
            .alias("fvg_fill_frac")
        )
        ...
        df = df.with_columns(
            pl.when(pl.col("fvg_direction") == pl.lit("none"))
            .then(pl.lit("none"))
            .when(pl.col("fvg_fill_frac") <= pl.lit(0.0))
            .then(pl.lit("unfilled"))
            .when(pl.col("fvg_fill_frac") >= pl.lit(1.0))
            .then(pl.lit("filled"))
            .when(pl.col("fvg_fill_frac") >= pl.lit(float(cfg.fvg_partial_fill_cut)))
            .then(pl.lit("partially_filled"))
            .otherwise(pl.lit("unfilled"))
            .alias("fvg_fill_state")
        )

        df = df.with_columns(
            (pl.when(pl.col("fvg_direction") != pl.lit("none")).then(1).otherwise(0))
            .cast(pl.Int32)
            .cumsum()
            .over("instrument")
            .alias("_fvg_seq")
        ...
        df = df.with_columns(
            pl.when((pl.col("fvg_fill_frac") > pl.lit(0.0)) & (pl.col("_fvg_seq") > 0))
            .then(pl.col("_bar_index"))
            .otherwise(pl.lit(None))
            .alias("_fvg_touch_idx")
        )

        df = df.with_columns(
            pl.col("_fvg_touch_idx")
            .min()
            .over(["instrument", "_fvg_seq"])
            .alias("_fvg_first_touch_idx")
        )

        df = df.with_columns(
            pl.when(pl.col("_fvg_first_touch_idx").is_null())
            .then(pl.lit(None))
            .otherwise((pl.col("_fvg_first_touch_idx") - pl.col("fvg_origin_idx")).cast(pl.Int32))
            .alias("fvg_fill_duration_bars")
        )

        df = df.with_columns(
            pl.when(pl.col("fvg_fill_duration_bars").is_not_null() & (pl.col("fvg_fill_duration_bars") <= pl.lit(int(cfg.fvg_max_fill_bars))))
            .then(pl.lit(True))
            .otherwise(pl.lit(False))
            .alias("_fvg_within_fill_window")
        )

        df = df.with_columns(
            pl.when(pl.col("fvg_fill_frac") > pl.lit(0.0))
            .then(pl.lit(True))
            .otherwise(pl.lit(False))
            .alias("fvg_was_mitigated_flag")
        )

        future_max_high = (
            pl.col("high")
            .shift(-1)
            .rolling_max(window_size=int(cfg.displacement_followthrough_bars), min_periods=1)
            .over("instrument")
            .alias("_future_max_high")
        )
        future_min_low = (
            pl.col("low")
            .shift(-1)
            .rolling_min(window_size=int(cfg.displacement_followthrough_bars), min_periods=1)
            .over("instrument")
            .alias("_future_min_low")
        )

        df = df.with_columns(future_max_high, future_min_low)

        df = df.with_columns(
            pl.when(pl.col("_fvg_first_touch_idx").is_not_null() & (pl.col("_bar_index") == pl.col("_fvg_first_touch_idx")))
            .then(pl.col("atr_anchor"))
            .otherwise(pl.lit(None))
            .alias("_fvg_touch_atr"),
            pl.when(pl.col("_fvg_first_touch_idx").is_not_null() & (pl.col("_bar_index") == pl.col("_fvg_first_touch_idx")))
            .then(pl.col("fvg_mid"))
            .otherwise(pl.lit(None))
            .alias("_fvg_touch_price"),
        )

        df = df.with_columns(
            pl.col("_fvg_touch_atr").forward_fill().over(["instrument", "_fvg_seq"]).alias("_fvg_touch_atr"),
            pl.col("_fvg_touch_price").forward_fill().over(["instrument", "_fvg_seq"]).alias("_fvg_touch_price"),
        )

        df = df.with_columns(
            pl.when(pl.col("fvg_direction") == pl.lit("bull"))
            .then(pl.col("_future_max_high") - pl.col("_fvg_touch_price"))
            .when(pl.col("fvg_direction") == pl.lit("bear"))
            .then(pl.col("_fvg_touch_price") - pl.col("_future_min_low"))
            .otherwise(pl.lit(0.0))
            .alias("_fvg_reaction_move")
        )

        df = df.with_columns(
            pl.when(
                (pl.col("_fvg_first_touch_idx").is_not_null())
                & (pl.col("_fvg_reaction_move") >= (pl.lit(float(cfg.fvg_min_reaction_atr)) * pl.col("_fvg_touch_atr")))
                & (pl.col("_fvg_within_fill_window"))
            )
            .then(pl.lit(1.0))
            .otherwise(pl.lit(0.0))
            .alias("fvg_quality")
